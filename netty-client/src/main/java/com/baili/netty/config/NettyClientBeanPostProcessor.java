...
public class NettyClientBeanPostProcessor implements BeanPostProcessor {
    ...
    public NettyClientBeanPostProcessor(NettyClient nettyClient, NettyRpcProperties properties) {
        ...
        do {
            ...
            for (Field field : fields) {
                ...
                if (field.getAnnotation(RpcServer.class) != null) {
                    ...
                    try {
                        //通过JDK动态代理获取该类的代理对象
                        Object o = Proxy.newProxyInstance(field.getType().getClassLoader(), new Class[]{field.getType()}, new ClientInvocationHandle(nettyClient, properties));
                        //将代理类注入该字段
                        field.set(bean, o);
    ...
    static class ClientInvocationHandle implements InvocationHandler {
        ...
        public ClientInvocationHandle(NettyClient nettyClient, NettyRpcProperties properties) {
            this.nettyClient = nettyClient;
            this.properties = properties;
        }
        ...
        public Object invoke(Object proxy, Method method, Object[] args) {
            ...
            RpcMessage rpcMessage = RpcMessage.builder()
                    ...
                    .build();
            //调用Netty，发送数据
            RpcMessage send = nettyClient.send(properties.getServerPort(), rpcMessage);
            log.info("接收到服务端数据：{} \n返回结果值 ====》》》》{}", send, send.getResult());
